...
class CorEuclideanCholeskyMetric(CorFlatMetric):
    ...
    def deformation(self, C):
        n = C.shape[-1]
        k = min(self.k, n)
        idx = th.randperm(n, device=C.device)[:k]
        W = C[..., idx, :][..., :, idx]
        W = (W + W.transpose(-1, -2)) / 2
        I = th.eye(k, device=C.device, dtype=C.dtype).unsqueeze(0).expand(W.shape)
        Lw = th.linalg.cholesky(W + 1e-5 * I)
        C_sub = C[..., :, idx]
        X = th.linalg.solve(Lw, C_sub.transpose(-2, -1))
        L = X.transpose(-2, -1)
...
class CorLogEuclideanCholeskyMetric(CorEuclideanCholeskyMetric):
    ...
    def deformation(self, C):
        ...
        W = (W + W.transpose(-1, -2)) / 2
        I = th.eye(k, device=C.device, dtype=C.dtype).unsqueeze(0).expand(W.shape)
        Lw = th.linalg.cholesky(W + 1e-6 * I)
        C_sub = C[..., :, idx]
        X = th.linalg.solve(Lw, C_sub.transpose(-2, -1))
        L = X.transpose(-2, -1)
        L_full = th.zeros_like(C)
        L_full[..., :k] = L
        L_log = self.matrix_logarithm_lt1(L_full)
        return L_log.tril(-1)
