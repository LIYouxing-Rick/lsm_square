...
parser.add_argument('--nystrom-rank', default=50, type=int,
                    help='Nystrom rank k for ECM/LECM')
...
#parser.add_argument('--curvature', default=1.0, type=float,
                #    help='curvature c for Poincaré ball in PHCM')
...
parser.add_argument('--classifier-factor', default=5.0, type=float,
                    help='define the lr multiply factor for classifier')
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # reset param group learning rates to current config
            try:
                base_lrs = [args.lr, args.lr, args.lr*args.classifier_factor]
                ratio = 1.0
                try:
                    ratio = LR.lr[args.start_epoch] / LR.lr[0]
                except Exception:
                    ratio = 1.0
                for i, pg in enumerate(optimizer.param_groups):
                    if i < len(base_lrs):
                        pg['lr'] = base_lrs[i] * ratio
    ...
    if not args.distributed or dist.get_rank() == 0:
        try:
            ...
            with open(os.path.join(outdir, 'best_acc.txt'), 'a') as f:
                lr_params_str = ''
                try:
                    if isinstance(args.lr_params, list) and len(args.lr_params) > 0:
...
class Learning_rate_generater(object):
    ...
    def __init__(self, method, params, total_epoch):
        ...
        if m == 'step':
            ...
            lr_factor, lr = self.none(total_epoch)
        elif m == 'cosine':
            lr_factor, lr = self.cosine(total_epoch)
        else:
    ...
    def log(self, params, total_epoch):
        ...
        if lr[0] != args.lr:
            args.lr = lr[0]
        return lr_factor, lr
    def none(self, total_epoch):
        lr_factor = [1.0] * total_epoch
        lr = [args.lr]
        for _ in range(1, total_epoch):
            lr.append(args.lr)
        return lr_factor, lr
    def cosine(self, total_epoch):
        lr = []
        T = max(1, total_epoch - 1)
        for e in range(total_epoch):
            f = 0.5 * (1.0 + np.cos(np.pi * e / T))
            lr.append(args.lr * f)
        lr_factor = [1.0]
        for e in range(1, total_epoch):
            prev = lr[e-1]
            curr = lr[e]
            lr_factor.append(curr/prev if prev != 0 else 1.0)
        return lr_factor, lr
