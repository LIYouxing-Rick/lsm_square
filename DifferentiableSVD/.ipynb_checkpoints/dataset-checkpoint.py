...
class Cars(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        # 定义标注文件路径
        # 训练集标注（在 devkit 目录下）
        annos_train = os.path.join(self.root, 'devkit', 'cars_train_annos.mat')
        # 测试集标注（带标签版本，在根目录下）
        annos_test_with_labels = os.path.join(self.root, 'cars_test_annos_withlabels.mat')
        # 旧版测试集标注（无标签，在 devkit 目录下）
        annos_test_no_labels = os.path.join(self.root, 'devkit', 'cars_test_annos.mat')
        ...
        if train:
            # ============ 训练集：使用 cars_train_annos.mat ============
            if not os.path.exists(annos_train):
                raise RuntimeError(
                    f'Training annotations not found at {annos_train}!\n'
                    f'Expected structure:\n'
                    f'  {self.root}/devkit/cars_train_annos.mat\n'
                    f'  {self.root}/cars_train/ (or car_ims/)'
                )
            
            print(f'Loading training set from {annos_train}')
            loaded_mat = io.loadmat(annos_train)
            loaded_items = loaded_mat['annotations'][0]

            for item in loaded_items:
                fname = str(item['fname'][0])
                label = int(item['class'][0][0]) - 1
                candidate1 = os.path.join('cars_train', fname)
                candidate2 = os.path.join('car_ims', fname)
                if os.path.exists(os.path.join(self.root, candidate1)):
                    relative_path = candidate1
                elif os.path.exists(os.path.join(self.root, candidate2)):
                    relative_path = candidate2
                else:
                    relative_path = candidate1
                self.samples.append((relative_path, label))
            
            print(f'Loaded {len(self.samples)} training samples')
        
        else:
            # ============ 测试/验证集：使用 cars_test_annos_withlabels.mat ============
            if os.path.exists(annos_test_with_labels):
                # 优先使用带标签的测试集标注
                print(f'Loading test set from {annos_test_with_labels}')
                loaded_mat = io.loadmat(annos_test_with_labels)
                loaded_items = loaded_mat['annotations'][0]
                ...
                for item in loaded_items:
                    ...
                    # 尝试不同的字段名
                    try:
                        # 格式1: 有 'fname' 和 'class' 字段
                        fname = str(item['fname'][0])
                        label = int(item['class'][0][0]) - 1
                    except (ValueError, IndexError):
                        # 格式2: 与训练集相同的结构
                        fname = os.path.basename(str(item[0][0]))
                        label = int(item[4][0]) - 1
                    
                    relative_path = os.path.join('cars_test', fname)
                    self.samples.append((relative_path, label))
                
                print(f'Loaded {len(self.samples)} test samples')
            
            elif os.path.exists(annos_test_no_labels):
                # 如果只有无标签的测试集标注
                raise RuntimeError(
                    f'Test annotations without labels found at {annos_test_no_labels}.\n'
                    f'This version does not contain test labels!\n'
                    f'Please use the dataset version with cars_test_annos_withlabels.mat\n'
                    f'You can download it from: https://github.com/jhpohovey/StanfordCars'
                )
            else:
                raise RuntimeError(
                    f'Test annotations not found!\n'
                    f'Expected: {annos_test_with_labels}\n'
                    f'Please download the complete dataset.'
                )
    ...
    def _check_exists(self):
        """检查数据集的关键文件是否存在"""
        # 检查训练集标注
        train_annos = os.path.join(self.root, 'devkit', 'cars_train_annos.mat')
        if not os.path.exists(train_annos):
            return False
        
        # 检查训练集图片目录（可能是 cars_train 或 car_ims）
        train_dir1 = os.path.join(self.root, 'cars_train')
        train_dir2 = os.path.join(self.root, 'car_ims')
        if not (os.path.exists(train_dir1) or os.path.exists(train_dir2)):
            return False
        
        # 检查测试集图片目录
        test_dir = os.path.join(self.root, 'cars_test')
    ...
    def _download(self):
        ...
        print('Note: The official download links are broken.')
        print('Please manually download from: https://github.com/jhpohovey/StanfordCars')
        print('Or use: git clone https://github.com/jhpohovey/StanfordCars.git')
